...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        neighbours = []
        for i in range(-1,2):
            if cell[0]+i>=0 and cell[0]+i<self.height : #checks for bounds
                for j in range(-1,2):
                    if cell[1]+j>=0 and cell[1]+j<self.width:
                         if (cell[0]+i,cell[1]+j) not in self.moves_made: #checks for moves made
                             neighbours.append((cell[0]+i,cell[1]+j))
        #print("sentence",Sentence(neighbours,count))
        self.knowledge.append(Sentence(neighbours,count))

        #4
        nSentences = ["flag"]
        while nSentences: #keeps running until no more new sentences
            nSentences=[]
            for sentence in self.knowledge:
                ksafes = sentence.known_safes() 
                
                if ksafes:
                    copysafes = copy.deepcopy(ksafes)
                    for safem in copysafes:
                        self.mark_safe(safem)
                
                kmines=sentence.known_mines()
                
                if kmines:
                    copymines = copy.deepcopy(kmines)
                    for minem in copymines:
                        self.mark_mine(minem)
            ...
            for i in range(len(self.knowledge)):
                ...
                for j in range(i,len(self.knowledge)):
                    if self.knowledge[j].cells < self.knowledge[i].cells:
                        newS = Sentence(self.knowledge[i].cells-self.knowledge[j].cells,self.knowledge[i].count-self.knowledge[j].count)
                        if newS not in self.knowledge:
                            nSentences.append(newS)
                            #print("nSEncte",nSentences)
